(function(){
  ...
  Ship.prototype.rotateShip = function(ctx) { 
    ...
    var maxX = Math.sqrt(Math.pow(Math.cos(this.heading*Math.PI/180)*warpX,2) + Math.pow(Math.sin(this.heading*Math.PI/180)*warpY,2))
    var maxY = Math.sqrt(Math.pow(Math.sin(this.heading*Math.PI/180)*warpX,2) + Math.pow(Math.cos(this.heading*Math.PI/180)*warpY,2))
    // draw it up and to the left by half the width
    // and height of the image 
    ctx.drawImage(this.img, -(this.img.width/2), -(this.img.height/2), this.img.width*maxX, this.img.height*maxY);
