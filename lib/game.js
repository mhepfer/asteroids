(function(){
  ...
  Game.prototype.accelerateAsteroids = function(heading, impulse){
    ...
    var limSqr = Math.pow(this.velocityLimit,2)

    for (var i = 0; i < this.asteroids.length; i++) {
      var asteroid = this.asteroids[i]
      var norm = Math.sqrt(Math.pow(asteroid.vel[0],2) + Math.pow(asteroid.vel[1],2));
      var gamma = 1/Math.sqrt(1 - Math.pow(norm/this.velocityLimit,2))
      var dotProd = asteroid.vel[0]*addVelX+asteroid.vel[1]*addVelY
      var asteroidVelCoeff = 1-gamma*dotProd/(limSqr+limSqr*gamma)
      var addVelCoeff = 1/gamma
      if(isNaN(norm) || isNaN(gamma)){
        debugger
        asteroid.vel[0] = asteroid.vel[0] - 0.5
        asteroid.vel[1] = asteroid.vel[1] - 0.5
      } else {
        asteroid.vel[0] = (1/(1+dotProd/limSqr))*(asteroidVelCoeff*asteroid.vel[0]+addVelCoeff*addVelX);
        asteroid.vel[1] = (1/(1+dotProd/limSqr))*(asteroidVelCoeff*asteroid.vel[1]+addVelCoeff*addVelY);
      }
    }
  };
